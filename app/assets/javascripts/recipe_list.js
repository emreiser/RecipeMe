...
RecipeMe.set_up_recipe_ccontainer = function() {
  recipe_container_element = '<div id="recipes-index-content">';
};


// parses out the ingredients
RecipeMe.list_ingredients = function(basket) {
};

//Runs set_up_div_container and make API call for the recipies
RecipeMe.request_recipies = function(event, basket) {
  RecipeMe.set_up_recipe_ccontainer();
...
RecipeMe.render_all = function(recipes) {
  ...
  var number_recipes = recipes.length, i;
  for(i = 0; i < number_recipes; i++) {
    this.render_one(recipes[i], i);
  }
};
